...
using Serpentpro.Protections;
using Serpentpro.Services;
...
namespace Serpentpro
{
    class Program
    {
        public static void Main(string[] args)
        {
            ...
            Mutation.Execute(mod);
            AntiDumpExecute(mod);
            EraseHeadersExecute(mod);
            base64(mod);
            Rename(mod);
        ...
        public static void AntiDumpExecute(ModuleDef mod)
        {
            ...
            var typeDef = typeModule.ResolveTypeDef(MDToken.ToRID(typeof(AntiDump).MetadataToken));
        ...
        public static void EraseHeadersExecute(ModuleDef mod)
        {
            var typeModule = ModuleDefMD.Load(typeof(EraseHeaders).Module);
            var cctor = mod.GlobalType.FindOrCreateStaticConstructor();
            var typeDef = typeModule.ResolveTypeDef(MDToken.ToRID(typeof(EraseHeaders).MetadataToken));
            var members = InjectHelper.Inject(typeDef, mod.GlobalType, mod);
            var init = (MethodDef)members.Single(method => method.Name == "Initialize");
            cctor.Body.Instructions.Insert(0, Instruction.Create(OpCodes.Call, init));
            foreach (var md in mod.GlobalType.Methods)
        ...
        public static string Ascii = " 俺01234ム仮俺56789ム仮 ";
